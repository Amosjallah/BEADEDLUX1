    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="bg-sky-700 hover:bg-sky-800 text-white px-6 py-3 rounded-lg font-semibold transition-colors whitespace-nowrap cursor-pointer"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border-2 border-gray-200 p-4">
          ...
          <p className="text-2xl font-bold text-sky-700">{categories.filter(c => c.status === 'active').length}</p>
        </div>
        <div className="bg-white rounded-xl border-2 border-gray-200 p-4">
          <p className="text-sm text-gray-600 mb-1">Featured</p>
          <p className="text-2xl font-bold text-sky-700">{categories.filter(c => c.metadata?.featured).length}</p>
        </div>
      </div>
              ...
              {loading ? (
                ...
                categories.map((category) => (
                  <tr key={category.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-4 px-4">
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap capitalize ${category.status === 'active' ? 'bg-sky-100 text-sky-700' : 'bg-gray-100 text-gray-600'
                        }`}>
                        {category.status}
                      </span>
                    </td>
                    <td className="py-4 px-4">
                      {category.metadata?.featured ? (
                        <span className="px-3 py-1 bg-sky-100 text-sky-700 rounded-full text-xs font-semibold whitespace-nowrap">
                          Featured
                        </span>
                      ) : (
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center space-x-2">
                        <button
                          ...
                          className="w-8 h-8 flex items-center justify-center text-gray-600 hover:text-sky-700 hover:bg-sky-50 rounded-lg transition-colors cursor-pointer"
                        >
      ...
      {(showAddModal || showEditModal) && (
        <div className="fixed inset-0 bg-sky-600/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-sky-700 hover:bg-sky-50 transition-colors relative">
                  {uploading ? (
                    <div className="flex flex-col items-center">
                      <i className="ri-loader-4-line animate-spin text-3xl mb-2 text-sky-700"></i>
                      <span className="text-sm font-medium text-gray-600">Uploading...</span>
                    </div>
                  ) : formData.image_url ? (
                    <div className="relative group">
                      <img src={formData.image_url} alt="Category" className="h-40 mx-auto object-contain rounded-lg shadow-sm" />
                      <div className="absolute inset-0 bg-sky-600/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity rounded-lg">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500 cursor-pointer"
                  >
            ...
            <div className="p-6 border-t border-gray-200 flex items-center justify-end space-x-3 bg-gray-50 rounded-b-2xl">
              ...
              <button
                ...
                className={`px-6 py-3 bg-sky-700 hover:bg-sky-800 text-white rounded-lg font-semibold transition-colors whitespace-nowrap cursor-pointer flex items-center ${saving ? 'opacity-70' : ''}`}
              >
