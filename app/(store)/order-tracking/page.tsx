...
function OrderTrackingContent() {
  ...
  const getStatusBadge = () => {
    ...
    const statusMap: Record<string, { label: string; color: string }> = {
      ...
      'delivered': { label: 'Delivered', color: 'bg-sky-100 text-sky-800' },
          ...
          <div className="bg-white rounded-xl shadow-sm p-8">
            <form onSubmit={handleTrack} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-sky-700 hover:bg-sky-800 text-white py-4 rounded-lg font-semibold transition-colors whitespace-nowrap disabled:opacity-50"
              >
                {loading ? (
            ...
            <div className="mt-8 p-4 bg-sky-50 border border-sky-200 rounded-lg">
              <div className="flex items-start space-x-3">
                <i className="ri-information-line text-xl text-sky-700 mt-0.5"></i>
                <div>
                  <p className="text-sm font-semibold text-sky-900">Need Help?</p>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm p-8 mb-8">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-8">
            <div className="p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 flex items-center justify-center bg-sky-100 rounded-full">
                  <i className="ri-map-pin-line text-xl text-sky-700"></i>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Shipping To</p>
                  <p className="font-semibold text-gray-900">
                    {shippingAddress.city || shippingAddress.region || 'USA'}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 flex items-center justify-center bg-sky-100 rounded-full">
                  <i className="ri-money-cny-circle-line text-xl text-sky-700"></i>
                </div>
                <div>
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 flex items-center justify-center bg-sky-100 rounded-full">
                  <i className="ri-box-3-line text-xl text-sky-700"></i>
                </div>
                <div>
              ...
              <div key={step.key} className="flex items-start mb-8 last:mb-0">
                <div className="relative flex flex-col items-center mr-6">
                  <div className={`w-12 h-12 flex items-center justify-center rounded-full font-bold transition-colors ${
                    step.status === 'completed'
                      ...
                      : step.status === 'active'
                      ? 'bg-sky-100 text-sky-700 ring-4 ring-sky-200'
                  ...
                  {index < trackingSteps.length - 1 && (
                    <div className={`w-0.5 h-16 mt-2 ${
                      step.status === 'completed' ? 'bg-sky-700' : 'bg-gray-200'
                    }`}></div>
                  )}
                </div>
                <div className="flex-1 pt-2">
                  ...
                  <p className={`text-sm mt-1 font-semibold ${
                    step.status === 'pending' ? 'text-gray-400' : 'text-sky-700'
                  }`}>
                    {step.date}
                  </p>
                </div>
          ...
          <div className="space-y-4">
            {order.order_items?.map((item: any) => (
              <div key={item.id} className="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg">
                ...
                <p className="font-bold text-sky-700">$ {Number(item.unit_price).toFixed(2)}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-8 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link href="/contact" className="text-sky-700 hover:text-sky-900 font-semibold whitespace-nowrap">
              <i className="ri-customer-service-line mr-1"></i>
              Contact Support
            </Link>
            <Link href="/returns" className="text-sky-700 hover:text-sky-900 font-semibold whitespace-nowrap">
    ...
    <Suspense fallback={
      <main className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 border-4 border-sky-700 border-t-transparent rounded-full animate-spin mx-auto"></div>
        </div>
      </main>
    }>
