    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="px-4 py-2 bg-sky-700 text-white rounded-lg font-semibold hover:bg-sky-800 transition-colors whitespace-nowrap"
        >
      ...
      {showForm && (
        <div className="bg-white border-2 border-sky-700 rounded-lg p-6 mb-6">
          ...
          <form className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              <label className="flex items-center">
                <input type="checkbox" className="w-4 h-4 text-sky-700 border-gray-300 rounded focus:ring-sky-700" />
            ...
            <div className="md:col-span-2 flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
              <button
                type="submit"
                className="flex-1 py-3 bg-sky-700 text-white rounded-lg font-semibold hover:bg-sky-800 transition-colors whitespace-nowrap"
              >
                Save Address
              </button>
              <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {addresses.map((address) => (
          <div
            key={address.id}
            className={`bg-white border-2 rounded-lg p-6 relative ${address.isDefault ? 'border-sky-700' : 'border-gray-200'
              }`}
          >
            {address.isDefault && (
              <div className="absolute top-4 right-4">
                <span className="px-3 py-1 bg-sky-700 text-white text-xs font-semibold rounded-full whitespace-nowrap">
                  Default
                </span>
              </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
              ...
              {!address.isDefault && (
                <button
                  ...
                  className="flex-1 py-2 border border-sky-700 text-sky-700 rounded-lg font-semibold hover:bg-sky-50 transition-colors whitespace-nowrap"
                >
                  Set Default
                </button>
              )}
