        ...
        {items.map((item) => (
          <div key={`${item.id}-${item.variant || 'novar'}`} className="flex space-x-4">
            <div className="relative w-20 h-20 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
              ...
              />
              <div className="absolute -top-2 -right-2 w-6 h-6 flex items-center justify-center bg-sky-700 text-white text-xs font-bold rounded-full">
                {item.quantity}
              </div>
            </div>
            <div className="flex-1">
              ...
              <p className="text-sky-700 font-bold mt-1">$ {item.price.toFixed(2)}</p>
            </div>
          </div>
        ))}
      </div>
