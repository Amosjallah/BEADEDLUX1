...
export default function CartSuggestions() {
  ...
  return (
    <div className="bg-sky-50 border-2 border-sky-100 rounded-lg p-6 mb-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-bold text-gray-900">You Might Also Like</h3>
        <span className="text-sm text-sky-700 font-medium whitespace-nowrap">Boost Your Order</span>
      </div>
            ...
            <Link href={`/product/${product.id}`}>
              ...
              <div className="p-3">
                ...
                <div className="flex items-center space-x-2 mb-2">
                  <span className="text-lg font-bold text-sky-700">${product.price.toFixed(2)}</span>
                  {product.originalPrice && (
                    <span className="text-xs text-gray-400 line-through">${product.originalPrice.toFixed(2)}</span>
                  )}
                </div>
                <button className="w-full py-2 bg-sky-700 text-white text-sm rounded-lg font-semibold hover:bg-sky-800 transition-colors whitespace-nowrap">
